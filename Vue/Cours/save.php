                ...
                <div class="bg-white border border-gray-300 rounded-lg overflow-hidden shadow-lg relative transition-none">
                    ...
                    <div class="absolute bottom-4 right-4 sm:static sm:flex sm:justify-end sm:p-4">
                        <div class="flex flex-col sm:flex-row sm:space-x-4 sm:space-y-0 space-y-2">
                            <a class="flex items-center bg-orange-400 h-10 px-3 mt-2 rounded hover:bg-gray-300 sm:mt-0 sm:ml-2" href="index.php?uc=eleve&action=inscription&idprof=<?= $cours->IDPROF ?>&nums=<?= $cours->NUMSEANCE ?>&jour=<?= $cours->JOUR ?>&tranche=<?= $cours->TRANCHE ?>">
                            ...
                            <a class="flex items-center bg-yellow-500 h-10 px-3 mt-2 rounded hover:bg-yellow-600 sm:mt-0 sm:ml-2" href="index.php?uc=inscriptions&action=liste&idprof=<?= $cours->IDPROF ?>&nums=<?= $cours->NUMSEANCE ?>&jour=<?= $cours->JOUR ?>&tranche=<?= $cours->TRANCHE ?>">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
                                    <circle cx="8.5" cy="7" r="4"></circle>
                                    <line x1="20" y1="8" x2="20" y2="14"></line>
                            ...
                            <a class="flex items-center bg-blue-900 h-10 px-3 mt-2 rounded hover:bg-gray-300 sm:mt-0 sm:ml-2" href="index.php?uc=inscriptions&action=liste&idprof=<?= $cours->IDPROF ?>&nums=<?= $cours->NUMSEANCE ?>&jour=<?= $cours->JOUR ?>&tranche=<?= $cours->TRANCHE ?>">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
